...
import { ISaveFlow, IWorkflow, IWorkflowBase } from "@/interface/workflow";

export const getFlows = async (type: 'WORKFLOW' | "TEMPLATE" = 'WORKFLOW') => {
    const flows = (await apiCaller.get<ApiResult<IWorkflow[]>>(`${process.env.NEXT_PUBLIC_FLOWS_API}/${type}`)).data.data;
    return flows || []
}

export const getFlow = async (id?: string) => {
    if (!id) return undefined
    const flow = await (await apiCaller.get<ApiResult<IWorkflow>>(`${process.env.NEXT_PUBLIC_FLOW_API}/${id}`)).data.data
    return flow
}
